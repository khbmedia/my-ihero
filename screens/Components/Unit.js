...
const Unit = props => {
    ...
    const deleteColor = (id) => {
        axios.get('https://depot25.dev.khb.asia/api/front-product-unit-destroy/' + id)
            .then(res => {
                console.log('success');
                setShowModal(false);
            })
    ...
    const editColor = (ele) => {
        navigation.navigate("ProductStack", { screen: "UnitsStack", params: { screen: 'CreateUnits', params: { ele } } });
    }
    return props.UnitList.map
        (ele =>
            <TouchableOpacity style={styles.borderitem} key={ele.id} onPress={() => editColor(ele)}>
                <List.Item style={props.UnitList}>
                            ...
                            <Modal.Content maxWidth="400px" style={{ backgroundColor: '#060818' }}>
                                ...
                                <Modal.Footer>
                                    <Button.Group variant="ghost" space={2}>
                                        <Button onPress={() => { deleteColor(ColorID); }}  >
