...
const ListVideo = ({ navigation }) => {
    ...
    useEffect(() => {
        setTimeout(() => {
            axios.get('https://depot25.dev.khb.asia/api/front-product-video-index')
                .then(res => {
    ...
    return (
        <View style={{ backgroundColor: 'white', width: '100%', flex: 1, justifyContent: 'center', }} >
            ...
            <NativeBaseProvider>
                <TouchableOpacity style={{ borderRadius: 10, alignItems: 'center' }} onPress={() => navigation.navigate("CreateVideos")}>
                    <View style={{ borderColor: 'gray', borderWidth: 0.3, marginTop: 13, width: '98%', }}>
                        ...
                        <Text style={{ fontSize: 15, color: '#666666', textAlign: 'center' }}>Add Video</Text>
                    </View>
                </TouchableOpacity>
                <ScrollView  >
